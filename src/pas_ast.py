...
class FileAST(Node):
    ...
    def children(self):
        nodelist = []
        if self.ext is not None: nodelist.append(self.ext)
...
class Unit(Node):
    def __init__(self, name, portability_opt, interfacesection, implementationsection, initsection, coord=None):
        self.name = name
        self.portability_opt = portability_opt
        self.interfacesection = interfacesection
        self.implementationsection = implementationsection
        self.initsection = initsection
        self.coord = coord

    def children(self):
        nodelist = []
        if self.interfacesection is not None: nodelist.append(self.interfacesection)
        if self.implementationsection is not None: nodelist.append(self.implementationsection)
        if self.initsection is not None: nodelist.append(self.initsection)
        return tuple(nodelist)
    ...
    def show(self, buf=sys.stdout, offset=0, attrnames=False, showcoord=False):
        ...
        if attrnames:
            attrstr = ', '.join('%s=%s' % nv for nv in [("name", repr(self.name)), ("portability_opt", repr(self.portability_opt))])
        else:
            attrstr = ', '.join('%s' % v for v in [self.name, self.portability_opt])
        buf.write(attrstr)
...
class InterfaceSection(Node):
    def __init__(self, uses_clause, interfacedeclarations, coord=None):
        self.uses_clause = uses_clause
        self.interfacedeclarations = interfacedeclarations
        self.coord = coord

    def children(self):
        nodelist = []
        if self.uses_clause is not None: nodelist.append(self.uses_clause)
        if self.interfacedeclarations is not None: nodelist.extend(self.interfacedeclarations)
        return tuple(nodelist)

    def show(self, buf=sys.stdout, offset=0, attrnames=False, showcoord=False):
        lead = ' ' * offset
        buf.write(lead + 'InterfaceSection: ')
...
class ImplementationSection(Node):
    def __init__(self, uses_clause, declsectionlist, exportsstmtlist, coord=None):
        self.uses_clause = uses_clause
        self.declsectionlist = declsectionlist
        self.exportsstmtlist = exportsstmtlist
        self.coord = coord

    def children(self):
        nodelist = []
        if self.uses_clause is not None: nodelist.append(self.uses_clause)
        if self.declsectionlist is not None: nodelist.extend(self.declsectionlist)
        if self.exportsstmtlist is not None: nodelist.extend(self.exportsstmtlist)
        return tuple(nodelist)

    def show(self, buf=sys.stdout, offset=0, attrnames=False, showcoord=False):
        lead = ' ' * offset
        buf.write(lead + 'ImplementationSection: ')
...
class UsesClause(Node):
    def __init__(self, uses, coord=None):
        self.uses = uses
        self.coord = coord
    ...
    def show(self, buf=sys.stdout, offset=0, attrnames=False, showcoord=False):
        lead = ' ' * offset
        buf.write(lead + 'UsesClause: ')

        if attrnames:
            attrstr = ', '.join('%s=%s' % nv for nv in [("uses", repr(self.uses))])
        else:
            attrstr = ', '.join('%s' % v for v in [self.uses])
        buf.write(attrstr)
...
class ConstSection(Node):
    def __init__(self, constantdecl_list, coord=None):
        self.constantdecl_list = constantdecl_list
        self.coord = coord

    def children(self):
        nodelist = []
        if self.constantdecl_list is not None: nodelist.extend(self.constantdecl_list)
        return tuple(nodelist)

    def show(self, buf=sys.stdout, offset=0, attrnames=False, showcoord=False):
        lead = ' ' * offset
        buf.write(lead + 'ConstSection: ')
...
class TypeSection(Node):
    def __init__(self, typedecl_list, coord=None):
        self.typedecl_list = typedecl_list
        self.coord = coord

    def children(self):
        nodelist = []
        if self.typedecl_list is not None: nodelist.extend(self.typedecl_list)
        return tuple(nodelist)

    def show(self, buf=sys.stdout, offset=0, attrnames=False, showcoord=False):
        lead = ' ' * offset
        buf.write(lead + 'TypeSection: ')
...
class ConstDecl(Node):
    ...
    def show(self, buf=sys.stdout, offset=0, attrnames=False, showcoord=False):
        lead = ' ' * offset
        buf.write(lead + 'ConstDecl: ')

        if attrnames:
            attrstr = ', '.join('%s=%s' % nv for nv in [("ident", repr(self.ident))])
        else:
            attrstr = ', '.join('%s' % v for v in [self.ident])
        buf.write(attrstr)
...
class VarDecl(Node):
    def __init__(self, ident, type, vardecl, portabilitydirective, coord=None):
        self.ident = ident
        self.type = type
        self.vardecl = vardecl
        self.portabilitydirective = portabilitydirective
        self.coord = coord

    def children(self):
        nodelist = []
        if self.vardecl is not None: nodelist.append(self.vardecl)
        if self.portabilitydirective is not None: nodelist.append(self.portabilitydirective)
        return tuple(nodelist)

    def show(self, buf=sys.stdout, offset=0, attrnames=False, showcoord=False):
        lead = ' ' * offset
        buf.write(lead + 'VarDecl: ')

        if attrnames:
            attrstr = ', '.join('%s=%s' % nv for nv in [("ident", repr(self.ident)), ("type", repr(self.type))])
        else:
            attrstr = ', '.join('%s' % v for v in [self.ident, self.type])
        buf.write(attrstr)
...
class VarSection(Node):
    def __init__(self, vardecls, coord=None):
        self.vardecls = vardecls
        self.coord = coord

    def children(self):
        nodelist = []
        if self.vardecls is not None: nodelist.extend(self.vardecls)
        return tuple(nodelist)

    def show(self, buf=sys.stdout, offset=0, attrnames=False, showcoord=False):
        lead = ' ' * offset
        buf.write(lead + 'VarSection: ')
...
class ProcedureDecl(Node):
    def __init__(self, heading, portability, proc_block, coord=None):
    ...
    def children(self):
        nodelist = []
        if self.heading is not None: nodelist.append(self.heading)
        if self.portability is not None: nodelist.append(self.portability)
        if self.proc_block is not None: nodelist.append(self.proc_block)
        return tuple(nodelist)

    def show(self, buf=sys.stdout, offset=0, attrnames=False, showcoord=False):
        lead = ' ' * offset
        buf.write(lead + 'ProcedureDecl: ')
...
class ConstructorDecl(Node):
    ...
    def show(self, buf=sys.stdout, offset=0, attrnames=False, showcoord=False):
        lead = ' ' * offset
        buf.write(lead + 'ConstructorDecl: ')
...
class ProcedureHeading(Node):
    def __init__(self, name, formalparameters, coord=None):
        self.name = name
    ...
    def children(self):
        nodelist = []
        if self.formalparameters is not None: nodelist.extend(self.formalparameters)
        return tuple(nodelist)

    def show(self, buf=sys.stdout, offset=0, attrnames=False, showcoord=False):
        lead = ' ' * offset
        buf.write(lead + 'ProcedureHeading: ')
...
class FunctionHeading(Node):
    def __init__(self, name, formalparameters, returns, coord=None):
        self.name = name
        self.formalparameters = formalparameters
    ...
    def children(self):
        nodelist = []
        if self.returns is not None: nodelist.append(self.returns)
        if self.formalparameters is not None: nodelist.extend(self.formalparameters)
        return tuple(nodelist)

    def show(self, buf=sys.stdout, offset=0, attrnames=False, showcoord=False):
        lead = ' ' * offset
        buf.write(lead + 'FunctionHeading: ')
...
class FormalParam(Node):
    def __init__(self, param, specifier, coord=None):
        self.param = param
    ...
    def show(self, buf=sys.stdout, offset=0, attrnames=False, showcoord=False):
        lead = ' ' * offset
        buf.write(lead + 'FormalParam: ')

        if attrnames:
            attrstr = ', '.join('%s=%s' % nv for nv in [("specifier", repr(self.specifier))])
        else:
            attrstr = ', '.join('%s' % v for v in [self.specifier])
        buf.write(attrstr)
...
class CompoundStmt(Node):
    def __init__(self, stmtlist, coord=None):
        self.stmtlist = stmtlist
        self.coord = coord

    def children(self):
        nodelist = []
        if self.stmtlist is not None: nodelist.extend(self.stmtlist)
        return tuple(nodelist)

    def show(self, buf=sys.stdout, offset=0, attrnames=False, showcoord=False):
        lead = ' ' * offset
        buf.write(lead + 'CompoundStmt: ')
...
class Block(Node):
    def __init__(self, declsectionlist, exportsstmtlist, compoundstmt, exportsstmtlist2, coord=None):
        self.declsectionlist = declsectionlist
        self.exportsstmtlist = exportsstmtlist
        self.compoundstmt = compoundstmt
        self.exportsstmtlist2 = exportsstmtlist2
        self.coord = coord

    def children(self):
        nodelist = []
        if self.compoundstmt is not None: nodelist.append(self.compoundstmt)
        if self.declsectionlist is not None: nodelist.extend(self.declsectionlist)
        if self.exportsstmtlist is not None: nodelist.extend(self.exportsstmtlist)
        if self.exportsstmtlist2 is not None: nodelist.extend(self.exportsstmtlist2)
        return tuple(nodelist)
